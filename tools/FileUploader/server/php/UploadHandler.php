...
define ('categoryID' , $_SESSION['categoryID']);
...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true) {
        $this->options = array(
			...
			'upload_dir' => DIR_DOWNLOAD . '/museum/gallery/'.categoryID.'/',
			'upload_url' => HTTP_SERVER . '/museum/gallery/'.categoryID.'/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
	...
	{  
	    $name=categoryID."/".$whichimg;
		$add_to_db = $this->query("INSERT INTO image (name) VALUES ('$name')") or die(mysql_error());  
		return $add_to_db;  
	}
	
	function delete_img($delimg)  
	{  	$name=categoryID."/".$delimg;
		$delete_from_db = $this->query("DELETE FROM image WHERE name = '$name'") or die(mysql_error());  
