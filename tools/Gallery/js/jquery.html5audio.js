...
/*
 * HTML5 Audio Player with Playlist by Tean v2.0
 *
 * http://codecanyon.net/user/Tean/portfolio
 */
...
(function($) {
	...
	//elements
	var componentWrapper = $(settings.playerHolder);
	...
	var ativatePlaylistScroll=settings.ativatePlaylistScroll;
	...
	var artistData = player_mediaName.html();//text in song title scroll
	...
	function _processPlaylistItem(){
		...
		if(data_type == 'local'){
			...
			//console.log(url);
			var url = 'http://ajax.googleapis.com/ajax/services/feed/load?v=1.0&output=xml&num=10&callback=?&q='+ encodeURIComponent(data_path); 
			$.ajax({
				type: 'GET',
				url: url,
				dataType: 'jsonp',
	...
	function createNodes(){
		if(processPlaylistDataArr.length){
			...
			for(i;i<len;i++){
				li = $('<li class= "playlistItem" data-type="'+processPlaylistDataArr[i].type+'" data-mp3Path="'+processPlaylistDataArr[i].mp3+'" data-oggPath="'+processPlaylistDataArr[i].ogg+'"></li>').addClass('playlistItem').appendTo(lastPlaylist);
	...
	function createNewPlaylist(){
		...
		if(ativatePlaylistScroll)checkScroll();
	...
	function resetData(){
	  ...
	  player_mediaName.html(artistData);
	...
	function getTitle(c){
		...
		if(isEmpty(playlistDataArr[c].title)){
			player_mediaName.html(artistData);//restore original
			return false;	
		}
	...
	*/
	this.reinitPlaylistScroll = function() {
		if(!componentInited) return false;
		if(playlistTransitionOn) return false;
		if(!lastPlaylist) return false;
		if(scrollPaneApi){
	...
	this.reinitNameScroll = function() {
		if(!componentInited) return false;
		if(playlistTransitionOn) return false;
		if(!lastPlaylist) return false;
		if(textScroller && useSongNameScroll){
